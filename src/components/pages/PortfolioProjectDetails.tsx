...
export function ProjectDetails({ projectId }: { projectId: string }) {
  ...
  const getShortDesc = (p: Project) =>
    isPt && p.shortDescriptionPt ? p.shortDescriptionPt : p.shortDescription;
  const getFullDesc = (p: Project) =>
    isPt && p.fullDescriptionPt ? p.fullDescriptionPt : p.fullDescription;
  ...
  const renderInstitutionBadge = (institution?: string) => {
    ...
    const logos: Record<string, string> = {
      ...
      "Harvard University":
        "/images/logos/Harvard_University_coat_of_arms150px.png",
    };
  ...
  return (
    <div className="pt-32 pb-20 min-h-screen">
      <div className="container mx-auto px-6">
        <div className="display-under-1023px flex items-center gap-4 text-zarco-cyan font-bold tracking-[0.3em] text-[10px] uppercase">
          <span>{project.category}</span>
          {project.category?.toLowerCase() === "college" &&
            project.institution &&
            project.institution !== "none" && (
              <>
        ...
        <h1 className="display-under-1023px text-5xl md:text-6xl font-black uppercase tracking-tight leading-[0.9] mt-3 mb-3">
          {getTitle(project)}
        </h1>
        <motion.button
        ...
        >
          ...
          <span className="text-[11px] font-bold uppercase tracking-widest">
            {isPt ? "Voltar ao Portfólio" : "Back to Portfolio"}
          </span>
        </motion.button>
        ...
        <div className="flex flex-col lg:flex-row gap-16">
                    ...
                    >
                      ...
                      <div className="absolute top-8 left-8 right-8 flex justify-between items-start pointer-events-none">
                        ...
                        <div className="pointer-events-auto">
                          {project.category?.toLowerCase() === "college" &&
                            renderInstitutionBadge(project.institution)}
                        </div>
                      </div>
                        ...
                        {images.map((img, idx) => (
                          <button
                            ...
                            className={`relative aspect-video w-24 rounded-[10px] overflow-hidden border-2 transition-all flex-shrink-0 ${
                              idx === currentImageIdx
                                ? "border-zarco-cyan scale-95 opacity-100 shadow-xl"
          ...
          <div className="w-full lg:w-2/5 lg:sticky lg:top-40 h-fit space-y-12">
            <motion.div
              ...
              className="space-y-6"
            >
              <div className="display-desktop flex items-center gap-4 text-zarco-cyan font-bold tracking-[0.3em] text-[10px] uppercase">
                <span>{project.category}</span>
                {project.category?.toLowerCase() === "college" &&
                  project.institution &&
                  project.institution !== "none" && (
              ...
              <h1 className="display-up-1023px text-5xl md:text-6xl font-black uppercase tracking-tight leading-[0.9]">
                {getTitle(project)}
              </h1>
            </motion.div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-3">
                  {project.techStack.map((tech, idx) => (
                    <span
                      key={idx}
                      className="px-5 py-3 bg-white/5 rounded-2xl text-[11px] font-black uppercase tracking-widest text-white/70 border border-white/5"
                    >
                      {tech}
                    </span>
                  ))}
            ...
            >
              {project.liveUrl && (
                ...
                >
                  ...
                  {isPt ? "Ver Site ao Vivo" : " View Live Site"}
                </a>
              )}
              {project.repoUrl && (
                <a
